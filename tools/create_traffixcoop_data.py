...
from tools.data_converter.traffixcoop_converter import traffixCoop2Nuscenes
from tools.data_converter.create_traffix_gt_database import (create_groundtruth_database)

def traffixcoop_data_prep(root_path,
                          info_prefix,
                          out_dir,
                          workers,
    ...
    converter = traffixCoop2Nuscenes(splits, load_dir, save_dir)
    converter.convert()
    if 'training' in splits or 'validation' in splits:
        print("creating groundtruth database")
        create_groundtruth_database("TraffixNuscCoopDataset", save_dir, info_prefix, f'{save_dir}/{info_prefix}_infos_train.pkl')
...
parser.add_argument(
    '--root-path',
    type=str,
    default='./data/traffix_coop',
    help='specify the root path of dataset')
parser.add_argument(
    '--out-dir',
    type=str,
    default='./data/traffix_coop_processed',
    required=False,
...
if __name__ == '__main__':
    traffixcoop_data_prep(
        root_path=args.root_path,
        info_prefix='traffix_nusc_coop',
