    ...
    def convert(self):
        ...
        for split in self.splits:
            ...
            if test:
                ...
                info_path = os.path.join(self.save_dir, "{}_infos_test.pkl".format('traffix_nusc'))
                mmcv.dump(data, info_path)
            else:
                if split == 'training':
                    print("train sample: {}".format(len(infos_list)))
                    data = dict(infos=infos_list, metadata=metadata)
                    info_path = os.path.join(self.save_dir, "{}_infos_train.pkl".format('traffix_nusc'))
                    mmcv.dump(data, info_path)
                elif split == 'validation':
                    print("val sample: {}".format(len(infos_list)))
                    data = dict(infos=infos_list, metadata=metadata)
                    info_path = os.path.join(self.save_dir, "{}_infos_val.pkl".format('traffix_nusc'))
                    mmcv.dump(data, info_path)
